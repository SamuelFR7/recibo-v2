...
export function PrintListDialog({ farms }: PrintListDialogProps) {
  ...
  function handleSubmit(data: Input) {
    window.open(
      `${env.VITE_API_URL}/api/relatoriolistagem?FazendaId=${data.farmId}`
    )
    form.reset()
    setIsOpen(false)
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className={cn('w-full md:w-[190px]')}>
